#Desafio 20, comandos desse desafio na linha 160
#Listas,dicionário e tupla
form,nomes,idades,telefones,nmpais,nmmaes,volt,permitir={},[],[],[],[],[],[],(12,13,14,15,16,17,18)

#Funções criadas
try:
    arq=open('tributos.txt','a',encoding='UTF-8')
    arq.close()
except:
...
def add0(x,y):
    x.append(str(input(f"Confirme para {y}: ")))
    form[y]=x
    arquivo=open('tributos.txt','a')
...
def add(x,y):
    ...
    form[y]=x
    abre=open('tributos.txt','a')
    abre.write(str(x[-1]+';').capitalize())
    abre.close()
    ...
    if (opc >= 1) and (opc <= 6):
        ...
        if opc == 4:
            if form == {}:
                ...
                #Comandos do desafio 20
                print("\033[31mAlistamento\033[m") 
                arquivoA = open('tributos.txt', 'r+',encoding='UTF-8')
                dados = arquivoA.readlines() 
                contador = [] 
                palavra=''
                for linha in dados: 
                    palavra = linha.strip()
                    contador.append(palavra)
                    quantidade=contador.count(palavra)
                print(f'\033[32m{nomes[-1]} alistado {quantidade} vez/vezes!\033[m')
